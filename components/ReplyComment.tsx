...
import { AppCommentContext, initialStateType, repliesType } from '../provider/AppProvider'
import { getDatabase , ref , set ,get , remove , update , push  } from "firebase/database";
import { database } from '../firebase/firebaseConfig';
export interface Proptype{
    comments:repliesType;
    rootCommentId:string
}

function ReplyComment({comments , rootCommentId}:Proptype) {
    ...
    useEffect(()=>{
        if(editTextAreaRef.current){
            editTextAreaRef.current.value = '@' + comments?.replyingTo + ', ' + comments?.content;
    ...
    },[comments.content,comments.replyingTo,isEditClick])

    useEffect(()=>{
        if(textAreaRef.current){
            textAreaRef.current.value = '@' + comments?.user?.username + ', ';
        }
    },[comments.user,isReplyClick])

    async function handleVotePlus(){
        let snapshotScorePlus = await get(ref( database ,`comments/${rootCommentId}/replies/${comments.id}/scorePlus`));
        let snapshotScoreMinus = await get(ref( database ,`comments/${rootCommentId}/replies/${comments.id}/scoreMinus`));
        ...
        for(let idMinus in userVoteMinusObj){
            if(userVoteMinusObj.hasOwnProperty(idMinus)){
                if(userVoteMinusObj[idMinus] === stateValue.currentUser.username){
                    remove(ref(database , `comments/${rootCommentId}/replies/${comments.id}/scoreMinus/${idMinus}`))
                }
            }
        }
        ...
        if(!userPlusList.includes(stateValue.currentUser.username)){
            ...
            await update(ref(database , `comments/${rootCommentId}/replies/${comments.id}/scorePlus`), updates);
        }else{
            for(let idPlus in userVotePlusObj){
                if(userVotePlusObj.hasOwnProperty(idPlus)){
                    if(userVotePlusObj[idPlus] === stateValue.currentUser.username){
                        remove(ref(database , `comments/${rootCommentId}/replies/${comments.id}/scorePlus/${idPlus}`))
                    }
                }
            }
        }
    ...
    async function handleMinusVote(){
        ...
        if(!userMinusList.includes(stateValue.currentUser.username)){
            ...
            const updates = {} as any;
            updates[dateNowInSec] = stateValue.currentUser.username;
            await update(ref(database , `comments/${rootCommentId}/replies/${comments.id}/scoreMinus`), updates);
        }else{
            for(let idMinus in userVoteMinusObj){
                if(userVoteMinusObj.hasOwnProperty(idMinus)){
                    if(userVoteMinusObj[idMinus] === stateValue.currentUser.username){
    ...
    async function handleReply(){
        if(textAreaRef.current){
            ...
            await update(ref(database , `comments/${rootCommentId}/replies/${date.getTime()}`),newReply);
    ...
    async function handleEdit(){
        if(editTextAreaRef.current && editTextAreaRef.current.value !== ''){
            ...
            newComment.content = editTextAreaRef.current.value.replace(`@${comments?.replyingTo},`,"");;
            ...
            await update(ref(database , `comments/${rootCommentId}/replies/${comments?.id}`),newComment);
    ...
    return (
        <div className="w-full flex flex-col items-center gap-2">
                <div className="flex bg-white rounded-lg w-full max-w-[610px] p-5 gap-5">
                    ...
                    <div className="w-full flex flex-col gap-4">
                            ...
                            <div className="hidden md:flex items-center gap-5">
                                {comments?.user?.username === stateValue.currentUser.username  &&
                                    <div className="flex gap-2 items-center cursor-pointer"
                                        ...
                                        onClick={()=>{
                                            setDeletePost(`comments/${rootCommentId}/replies/${comments?.id}`);
                                        }}
                                    >
                        ...
                        {isEditClick ? 
                            ...
                            :<p className="w-full max-w-[480px] text-[#80838a] text-[16px] break-words"><span className="text-[#706fb2] font-semibold">@{comments?.replyingTo+ " "}</span>{comments.content}</p>
                        }
                        <div className="flex md:hidden w-full justify-between items-center">
                            <div className="flex md:hidden w-[95px] h-10 justify-center rounded-xl bg-[#f5f6fa] items-center gap-3">
                                <div className="ease-linear duration-200 cursor-pointer p-1"
                                    onMouseEnter={()=>{setIsHoverPlus(true)}}
                                    onMouseLeave={()=>{setIsHoverPlus(false)}}
                                    onClick={handleVotePlus}
                ...
                {isReplyClick && 
                        <div className="flex bg-white rounded-lg w-full p-5 gap-4 items-start justify-center">
                            <img className="w-10 rounded-full" src={stateValue.currentUser.imagePng} alt=""/>
                            <textarea ref={textAreaRef} placeholder="Add a comment..." className="w-full max-w-[390px] overflow-y-clip resize-none min-h-[90px] bg-transparent outline-none rounded-lg px-4 py-1 focus:border-[#797996] border-2"
                                onChange={(e:any)=>{
                                    e.target.style.height = "auto";
                                    if(e.target.scrollHeight > 128){
                                        e.target.style.height = "128px";
